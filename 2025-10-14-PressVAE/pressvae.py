...
class VAE4(nn.Module):
    def __init__(self, in_ch=3, z_ch=64, base_ch=32):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(base_ch, in_ch*2, 3), NormAct(in_ch*2)             # -> (B,6,4,4)  // X'4
        )
    ...
    def decode(self, z):
        ...
        return self.out_mu(h), self.out_logsig(h), cond_feat
...
class PressVAE(nn.Module):
    def __init__(self):
        super().__init__()
        self.vae4 = VAE4(in_ch=3, z_ch=64, base_ch=32)
        self.vae8 = VAE8(in_ch=3, z_ch=64, base_ch=32, cond_in_ch=32)
        self.vae16 = VAE16(in_ch=3, z_ch=128, base_ch=32, cond_in_ch=32)
        self.vae32 = VAE32(in_ch=3, z_ch=128, base_ch=32, cond_in_ch=32)
...
def train_stage(
    stage: int,
    dataset: str = "cifar10",
    out_dir: str = "experiments/pressvae",
    epochs: int = 50,
    batch_size: int = 128,
    lr: float = 1e-3,
    ...
    def generate_at_stage(bs: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            if stage == 4:
                ...
                d32_mu, _, cond32 = model.vae32.decode(z32, cond16)
                gen = up16 + d32_mu
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, _ in loader:
            ...
            if stage == 4:
                o4 = model.vae4(X4)
                recon = gaussian_nll(X4, o4["x_mu"], o4["x_logsig"])
                ...
                recon = gaussian_nll(D8, o8["x_mu"], o8["x_logsig"])
                ...
                recon = gaussian_nll(D16, o16["x_mu"], o16["x_logsig"])
                ...
                recon = gaussian_nll(D32, o32["x_mu"], o32["x_logsig"])
...
def main():
    ...
    out_dir = f"experiments/pressvae_{timestamp}"
    ...
    p.add_argument("--dataset", type=str, choices=["cifar10", "mnist"], default="cifar10")
    args = p.parse_args()
